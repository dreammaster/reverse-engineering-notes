"""
Headless driver: run a target script against yendor2.idb via idat.exe, then
export yendor2.asm/.idc and save, all without opening the GUI.
...
The first idc.ARGV entry after the driver's own path is the target script to
exec (e.g. an apply_*.py rename/struct script, or a one-off fix/discovery
script). That script's own main()/module-level code runs exactly as it would
under Alt+F7 -- this wrapper just adds the analysis-wait, export, save, and
exit around it.

If the target script itself doesn't want an export (e.g. a pure read-only
discovery/report script like identify.py), pass a second ARGV entry of
"noexport" to skip the asm/idc export and database save steps:

    -S"batch_run_and_export.py identify.py noexport"

Every step is logged to batch_run_and_export.log via plain Python file I/O
rather than print()/msg() -- console output from idat.exe in -A mode is not
reliably flushed before exit, so the log file is the source of truth for what
happened on any given run, including exceptions. The target script's own
stdout is captured and appended to the log too.

Pattern carried over from the ultima1/ultima2 projects' ida_scripts drivers.
Two non-obvious things it needed there, and needs here too:
  ...
  - idat.exe's stdout/msg() output is not reliably flushed/visible before
    qexit(); don't depend on console output for diagnosis.
"""

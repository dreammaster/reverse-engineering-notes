"""
Read-only discovery script: reports what executable yendor2.idb corresponds
to and how much analysis work has already been done on it. Meant to be run
via run_ida_script.ps1 with -NoExport so it doesn't touch the database.
...
# Sample of already-named, non-sub_/non-library functions -- a first read on
# how much (if any) prior naming work in this IDB looks trustworthy vs. a
# placeholder/guess, per Paul's note that existing names shouldn't be assumed
# accurate.
print("\nnamed functions (non-sub_, first 200):")
count = 0
for ea in idautils.Functions():
    fname = idc.get_func_name(ea)
    if not fname.startswith("sub_"):
        print(f"  {ea:#08x}  {fname}")
        count += 1
        if count >= 200:
            print("  ...")
            break

...
`+0x52`/`+0x92`: a stat's current/max pair, confirmed as HP-like by
`CastSpell`'s heal codes (`0x12`/`0x13`: +25%/+50% of the missing
amount, capped at max); `+0x54`/`+0x94`: a second current/max pair
(plausibly MP), regenerated by `RestCharacter` the same way. Not yet
mapped: the 6 attribute values (`ShowCharacterStats`), the 8 item slots

...
this exact field pair is labeled "MAGIC:". Immediately before these,
three more field pairs — `+0x4C`/`+0x8C`, `+0x4E`/`+0x8E`,
`+0x50`/`+0x90` — are drawn by `DrawThreeThresholdStats` (via
`DrawValueWithThresholdColor`, called from `ShowCharacterSkills`) as
threshold-highlighted values; plausibly 3 primary attributes
(Strength/Dexterity/Stamina, going by the canonical order found via
`ShowArmorAttributeBonusList`), but which is which isn't confirmed.
`+0x58`,

...
RENAMES = [
    ...
    (0x1764A, "attemptExodusSequence",
     "**FULLY SOLVED, 2026-09-15** (previously confirmed at the "
     "mechanism level only). Reached via `off_1654C[0]` (`loc_1762F`: "
     "prompts 'Direct? ', reads a direction, and only proceeds if the "
     "tile there is `'|'` (0x7C) -- a control-panel/lever tile). "
     "Prompts a 4-choice menu 'D, S, L, M:\\n' (`EXODUS_SEQUENCE_MENU_KEYS` "
     "= 'M','D','L','S','Q',ESC), then checks THREE things against a "
     "running step counter (`word_164A0`, 0-3): (1) an index-"
     "consistency check on the chosen key's position in the menu "
     "table against the step number (`bl+0x1Eh` vs `ch`), (2) "
     "**the chosen letter (`al`) matches `EXODUS_SEQUENCE_ANSWER[word_164A0]`** "
     "(`[bx+6B14h]`, a 4-byte lookup array holding the literal bytes "
     "`'L','S','M','D'` -- confirmed by reading the raw bytes at "
     "linear 0x16B14, previously misdecoded as garbage code), and "
     "(3) the acting character's `_marksAndCards` "
     "(`[di+0Eh]`) has the CARD bit matching this step (`1 << "
     "word_164A0`) -- you must actually hold the matching Card (from "
     "obtainCard) to advance. All 3 matching advances `word_164A0`; a "
     "5-flash tile animation plays either way. Reaching "
     "`word_164A0 == 4` jumps straight to victorySequence. "
     "**THE ANSWER IS Love, Sol, Moons, Death (letters L, S, M, D, in "
     "that exact order)** -- confirmed two ways at once: directly from "
     "`EXODUS_SEQUENCE_ANSWER`'s own bytes, AND independently from the "
     "Time Lord's in-game vision text found in the same investigation "
     "(`aYouSeeAVisionO`, 'You see a vision of the Time Lord. He "
     "tells you: The one way is Love, Sol, Moons & Death, All else "
     "fails.') -- word-for-word the same sequence. This is Ultima "
     "III's legendary Exodus puzzle, now solved end-to-end from the "
     "disassembly: combat cannot defeat Exodus at all; only visiting "
     "4 specific panel positions in this exact order, each while "
     "holding its matching Card and speaking its name, does."),
    ...
    (0x15B51, "teleportPartyWithFanfare",
     ...
     "`WHIRLPOOL_X_TABLE`/`WHIRLPOOL_Y_TABLE` (named 2026-09-14 -- "
     "these are the exact tables the external file-formats doc guessed "
     "as 'moongate coordinates' at file offsets 0x184D/0x1855; "
     "confirmed here as the whirlpool's 8 possible on-map positions, "
     "not moongates) indexed by `word_11324`'s high byte, and "
    ...
    (0x120AE, "updateWhirlpoolPosition",
     ...
     "and when the position changes (via the same "
     "`WHIRLPOOL_X_TABLE`/`WHIRLPOOL_Y_TABLE` position-index tables "
     "teleportPartyWithFanfare reads): CORRECTED "
    ...
    (0x19630, "drawDungeonView",
     ...
     "nothing gets drawn). RESOLVED 2026-09-14 (was flagged as a "
     "self-modifying-code mystery): `start` (linear `0x10100`, the "
     "same 0x800-byte buffer entryFromBootup also uses as SOSARIA.ULT's "
     "load target) gets DUNGEON.DAT loaded directly into it by "
     "readAndDispatchCommand's dungeon-entry path ('lea bx, start; "
     "mov cx, 800h; lea dx, aDungeonDat; call loadFile'), right after "
     "the current dungeon's own numbered map file loads into "
     "byte_10900. DUNGEON.DAT's real on-disk bytes (confirmed via a "
     "raw hex dump, outside IDA) open with `e9 44 03` -- a jmp over an "
     "embedded data table to file offset 0x347 -- landing on genuine "
     "8086 code that saves the three passed-in registers, points ES "
     "at the CGA framebuffer segment (0B800h), and dispatches a "
     "sequence of wall-drawing calls at increasing corridor depths "
     "(al=0,1,3,6,0Ah). DUNGEON.DAT is not a data file: it IS the "
     "dungeon first-person-corridor-view renderer, shipped as a "
     "relocatable compiled-code overlay and loaded straight into the "
     "program's own entry-point buffer once that buffer is no longer "
     "needed for its original purpose. See file-formats.md."),

"""
Headless driver: run a target script against whichever ultima1*.idb was
passed to idat.exe, then export <same-stem>.asm/.idc and save, all without
opening the GUI.

Unlike ultima2 (a single .idb), this project has several IDBs -- one per
DOS executable (ultima1.idb, ultima1_gen.idb, ultima1_mondain.idb,
ultima1_out.idb, ultima1_space.idb) -- so this driver derives the export
paths from whatever IDB idat.exe actually opened (idc.get_idb_path()),
...
USAGE (from a shell, IDA GUI must be closed first -- the .idb gets locked):
    ...
    "C:\\Program Files\\IDA Pro 8.3\\idat.exe" -A ^
        -S"C:\\dev\\ultima1\\ida_scripts\\batch_run_and_export.py C:\\dev\\ultima1\\ida_scripts\\some_script.py" ^
        "C:\\dev\\ultima1\\ultima1_space.idb"

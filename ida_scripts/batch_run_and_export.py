"""
Headless driver: run a target script against whichever lota *.idb was
passed to idat.exe, then export <same-stem>.asm/.idc and save, all without
opening the GUI.

Legacy of the Ancients ships one thin compiled-BASIC .EXE per subsystem
(menu.idb, leglib.idb, out.idb, dun.idb, mus.idb, twndr.idb, ...) plus the
shared run-time module leglib.idb, so this driver derives the export paths
from whatever IDB idat.exe actually opened (idc.get_idb_path()), rather
than hardcoding a filename. (Pattern lifted verbatim from the sibling
ultima1 project's ida_scripts driver.)

USAGE (from a shell, IDA GUI must be closed first -- the .idb gets locked):

    "C:\\Program Files\\IDA Pro 8.3\\idat.exe" -A ^
        -S"C:\\dev\\lota\\ida_scripts\\batch_run_and_export.py C:\\dev\\lota\\ida_scripts\\some_script.py" ^
        "C:\\dev\\lota\\leglib.idb"
...
Two non-obvious things this pattern needed (carried over from ultima1 /
ultima2, confirmed against IDA Pro 8.3, idat.exe, 16-bit idb):

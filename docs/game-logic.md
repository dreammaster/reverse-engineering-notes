...
**Regular castle guard blow** — **live-confirmed** (DOSBox, 2026-09-06) —
`casdr.asm:4180` (`attackHit`), reached from `attackMiss` (`casdr.asm:4080`).
The castle turn handler runs each engaged guard's attack roll: `attackMiss`
first rolls to-hit against your **Dexterity** (miss ≈ `RND(1) < Dex/99`, so
higher Dex = more dodges), then on a hit calls `attackHit`:
```
raw = castleLevel^1.8 · (RND(1)·600 + 300) · difficulty      ' ds:2084 ^ ds:28B2, *ds:28B6, +ds:28BA
dmg = round( raw / ((ds:1AEC − 6) · Endurance^0.9) + 2 )     ' /armourTerm, +ds:2744; FF22 ROUNDS
hitPoints −= dmg   ;   dmg ≤ 0 check → if HP ≤ 0 you're thrown out
```
Endurance and armour (`ds:1AEC`, "armorId") mitigate as a **denominator**
term. `castleLevel` (`ds:2084`) = 1 on the public level, so `castleLevel^1.8`
drops out there; deeper levels scale the blow up. **Live capture** (PAULA,
castle L1, End 17, `ds:1AEC` 10, `ds:226E` = `0x3F800000` = 1.0): three
...
weapon HIT   when  RND(1) < (11·weaponId + 99)·(Dex + 13) / (7500·K)
                   K = 1.0 (castle)  or  Dexterity/26 (fort)   [ds:2214]
...
"`<enemy>` KILLED" at ≤ 0. `loadCastleLevel` sets `ds:2214` **and** `ds:226E`
from the same const: castle → `ds:25B0` = 1.0, fort → (`ds:2214` = Dex/26,
`ds:226E` = 3.5). *(`FF49` = `TOS / const` and `FF1F` reversed are both
confirmed from the leglib dispatch — so in the **fort**, `K = Dex/26` is a
real division and, with `K` in the denominator, higher Dex genuinely lowers
the fort hit-rate: an apparent original quirk, not a mis-read. In the
castle `K = 1` so Dex only helps.)*
...
| `ds:226E` (CASDR difficulty) | `loadCastleLevel` sets it: **castle → `ds:25B0` = 1.0, fort → `ds:31A8` = 3.5** (live-confirmed 2026-09-06 — was noted backwards). Same const also lands in `ds:2214` (the to-hit `K`). |
| `ds:20C0` (CASDR castleOrFort) | `1` = castle (`castle.bs1/2`), `2` = fort (`fort.bs1/2`); = `ds:1F20` = `(S4(12)==2)?2:1`. Live-confirmed `1` for the King's castle. |

...
| `+0x0B4` | 34 | **S3** `ds:1B3A` `DIM(16)` — 17 museum-progress words. `S3[k]` (k≈1–13) = **"viewed exhibit k"** (`PAULA` `[1..4]` all set after seeing 4 exhibits). `[14]` = used an exhibit portal (`mus.asm:1637`); `[15]` = museum entry count (`mus.asm:498`). A museum visit also **cleared PAULA's gem coins / "mail" / Seek spell from S2** — the museum may consume a coin per entry (or S2 re-syncs from a stale working copy on the transition). |

...
| `OUTDAT.DAT` | 1012 | `OUT` (disk 2) | **decoded** — the overworld **name table** (distinct from `OUTDATA.BSV`). 6-B header, then POOL 1 = 48 length-prefixed strings (`[0..23]` **24 place names** ZARYL/TARMALON/MERRILL/…, `[24..47]` **24 gem/stone names** DIAMOND/EMERALD/RUBY/…), then a 240-B numeric block (per-place/gem coords + a 24-word table + creature stat pairs, meanings TBD), then POOL 2 = **32 creature names** (PIXIE, STRIDER, FARMER, EATON WARRIOR, BANDIT, … MASTON LEAPER) that map **1:1 to the 32 `OUTDATA.BSV` sprite pairs**. Not opened by literal name (computed filename). `decoders/outdat_dat.py`. |

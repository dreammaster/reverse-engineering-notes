...
| `dun.idb` | `DUN.EXE` | 38 / 72 seg000 funcs (+ `rt_*` thunks) | 0 | Dungeon engine; chains back to `OUT`/`MUS`/`SAVER`. UNP-unpacked; 6 segments — **two** compiled-BASIC code segs: `seg000` "bmDUN" (main) + `seg001` "bmDUNG" (graphics helpers, 9 funcs), thunk table `seg002`, **DGROUP `seg004`**. Both coerced to ~100%. Named from the screen text + engine state vars (`apply_dsvars_dun.py` — `dungeonLevel`, `hitPoints`, `playerX/Y`, `tileAhead`, `selectedSpell`, `dungeonArrayPtr`, `turnActionFlag`/`chainDestType` [shared slots with OUT]): `dunMain`, `processTileFeature`, `moveMonsters`, `drawDungeonHud`, `doLookSearch`, `openChest`, `castSpell`, `loadDungeonLevel`, …. |

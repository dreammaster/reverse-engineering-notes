 ...
 * Originally written as (*((unsigned char *)(p))++) -- relying on the
 * long-removed GNU C extension that let you apply ++ directly to a cast
 * expression. p is always declared "unsigned char *" at every call site
 * in this file, so reordering the cast and the postfix ++ (which now
 * binds to the always-1-byte-stride p itself) gives byte-identical,
 * standard-conforming behavior on any modern compiler. */
#define READ_BYTE_NC(p)    (*(unsigned char *)(p)++)
#define READ_CHAR_NC(p)    (*(signed char *)(p)++)

#if (defined ALLEGRO_GCC) && (defined ALLEGRO_I386) && !(defined ALLEGRO_NO_ASM)

/* for gcc on i386 -- relies on a GNU C extension (treating a cast
 * expression as an lvalue for ++) that was removed from GCC long after
 * this code was written; the portable fallback below works fine on any
 * modern compiler, so this build (ALLEGRO_NO_ASM, see CMakeLists.txt)
 ...
 *
 * Originally called at every site as FLI_KLUDGE((char *)p, sz, ...) so that
 * the final "(p) = _fli_broken_data;" would type-check -- which relied on
 * the same now-removed GNU lvalue-cast extension as READ_BYTE_NC/
 * READ_CHAR_NC above (assigning through a cast expression). Every real
 * argument passed in is already some flavour of "unsigned char *", so the
 * call sites no longer cast, and the assignment here casts explicitly
 * instead of relying on the caller's cast becoming an lvalue. */

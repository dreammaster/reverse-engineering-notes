 ...
 * Line stretchers.
 *
 * DECLARE_STRETCHER/DECLARE_MASKED_STRETCHER originally advanced s via
 * "(unsigned char*) s += N" -- casting s to a byte pointer so it steps by
 * exactly N bytes regardless of type's own element size (needed: sinc/size
 * are always byte counts, but type is unsigned short/long for the 15/16/32
 * bit variants, where plain "s += N" would step by N*sizeof(type) bytes
 * instead). That relied on the same removed GNU lvalue-cast extension as
 * elsewhere in this codebase. The portable equivalent is an explicit
 * cast-to-char*-add-cast-back, reassigned to s.
 */
#define STEP_BYTES(s, n)  ((s) = (void *)((unsigned char *)(s) + (n)))
...
for (; d < dend; d += (size), STEP_BYTES(s, _al_stretch.sinc)) {          \
   set(d, get(s));                                                        \
   if (dd >= 0)                                                           \
      STEP_BYTES(s, (size)), dd += _al_stretch.i2;                        \
   else                                                                   \
      dd += _al_stretch.i1;                                               \
}

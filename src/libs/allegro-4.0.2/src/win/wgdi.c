...
/* function from asmlock.s/asmlock.c -- see the signature note on
 * gfx_directx_write_bank in wddraw.h; same reasoning applies here. */
extern unsigned long gfx_gdi_write_bank(BITMAP *bmp, int line);
extern void gfx_gdi_unwrite_bank(BITMAP *bmp);
...
/* hardware mouse cursor emulation.
 * gdi_mouse_sprite was originally just "mouse_sprite" -- a real, if
 * long-tolerated, naming collision in this file against the public
 * AL_VAR(BITMAP*, mouse_sprite) global declared in allegro/mouse.h
 * (a completely different bitmap: the currently-set cursor image, not
 * this driver's private emulation copy). Some older compilers apparently
 * let a file-scope static silently shadow/conflict with a same-named
 * extern; current GCC correctly rejects it, so it's renamed here. */
static int mouse_on = FALSE;
static int mouse_was_on = FALSE;
static BITMAP *gdi_mouse_sprite = NULL;
...
{
   if (gdi_mouse_sprite) {
      destroy_bitmap(gdi_mouse_sprite);
      gdi_mouse_sprite = NULL;
   ...
   gdi_mouse_sprite = create_bitmap(sprite->w, sprite->h);
   blit(sprite, gdi_mouse_sprite, 0, 0, 0, 0, sprite->w, sprite->h);
...
static void update_mouse_pointer(int x, int y, int retrace)
{
   ...
   draw_sprite(mouse_frontbuffer, gdi_mouse_sprite, 0, 0);
...
static void render_proc(void)
{
   ...
   if (top_line < gfx_gdi.h) {
      ...
      if (mouse_on) {
         if ((mouse_ypos+gdi_mouse_sprite->h > top_line) && (mouse_ypos <= bottom_line)) {

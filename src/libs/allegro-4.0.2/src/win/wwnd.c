...
static LRESULT CALLBACK directx_wnd_proc(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    ...
    * CallWindowProc's own prototype declares its first parameter as the
    * generic FARPROC, not WNDPROC -- passing WNDPROC without a cast used
    * to be a mere warning, but current GCC (14+) makes incompatible
    * pointer-type arguments a hard error by default. */
   if (user_wnd_proc)
      return CallWindowProc((FARPROC)user_wnd_proc, wnd, message, wparam, lparam);
   else

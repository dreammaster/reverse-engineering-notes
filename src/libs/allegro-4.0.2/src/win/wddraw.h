...
/* bitmap locking (from wddlock.c and asmlock.s/asmlock.c) */
...
/* Originally declared "(void)" -- correct only for the excluded i386-asm
 * build, where these are invoked through custom register-passing inline
 * asm trampolines (see al386gcc.h's bmp_write_line), never as an ordinary
 * C call. Under ALLEGRO_NO_ASM (this build), gfx.inl's bmp_write_line
 * casts bmp->write_bank to a plain "unsigned long (*)(BITMAP*, int)" and
 * calls it directly -- so the real, portable-C definitions in asmlock.c
 * need (and get) the ordinary-C-calling-convention signature here too. */
AL_FUNC(unsigned long, gfx_directx_write_bank, (BITMAP *bmp, int line));
AL_FUNC(void, gfx_directx_unwrite_bank, (BITMAP *bmp));
AL_FUNC(unsigned long, gfx_directx_write_bank_win, (BITMAP *bmp, int line));
